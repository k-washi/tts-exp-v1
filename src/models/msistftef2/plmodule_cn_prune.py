...
class MsISTFTEF2Module(LightningModule):
    ...
    def __get_prune_modules(self):
        prune_modules = []
        for name, module in self.net_g.named_modules():
            if isinstance(module, torch.nn.Conv1d) and "decoder.blocks" in name:
                prune_modules.append(module)
            elif isinstance(module, torch.nn.Linear) and "decoder.blocks" in name:
                prune_modules.append(module)
