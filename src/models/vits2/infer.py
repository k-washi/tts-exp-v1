...
from src.config.dataset.default import DataConfig
...
from src.models.core.components.flow import Flow
...
class TextEncoderOnnx(TextEncoder):
    ...
    def forward(
        ...
        if self.accent_emb is not None:
            accent_emb = self.accent_emb(accent_pos_padded)
            accent_emb = accent_emb.transpose(1, 2)
            # TODO: accent_weight < remove
            x = (
                self.emb(x) + self.accent_proj(accent_emb).transpose(1, 2)
            ) * math.sqrt(self.__input_embedding_dim)
        else:
...
class VitsInfer(nn.Module):
    def __init__(self,
                cfg: netG,
                data_cfg: DataConfig
        ):
        super(VitsInfer, self).__init__()
        self._cfg = cfg
