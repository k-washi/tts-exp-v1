...
from src.models.core.components.utils.residual import ResidualBlock
...
class HiFiGANGenerator(torch.nn.Module):
    ...
    def forward(self, c: torch.Tensor, g: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def inference(self, c: torch.Tensor, g: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Perform inference.
...
class HiFiGANMultiPeriodDiscriminator(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, disc:bool=False) -> tuple[List[torch.Tensor], List[torch.Tensor]]:
        """Calculate forward propagation.
...
class HiFiGANMultiScaleMultiPeriodDiscriminator(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, disc:bool=False) -> List[List[torch.Tensor]]:
        """Calculate forward propagation.
        ...
        """
        msd_outs, msd_input_list = self.msd(x, disc=disc)
        mpd_outs, mpd_input_list = self.mpd(x, disc=disc)
        return msd_outs + mpd_outs, msd_input_list + mpd_input_list