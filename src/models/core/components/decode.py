...
class Decoder(torch.nn.Module):
    def __init__(
        ...
        self.upsample_initial_channel = (
            upsample_initial_channel  # 入力されたzと埋め込み済み話者idのチャネル数を、まず最初にconvを適用させることによってupsample_initial_channelに揃える
        )
        ...
        self.conv1d_pre = nn.Conv1d(
            self.in_z_channel, self.upsample_initial_channel, kernel_size=7, stride=1, padding=3
        )
        ...
        for i in range(len(self.ups)):
            ...
            for j, (kernel, dilation) in enumerate(zip(self.resblock_kernel_sizes, self.resblock_dilation_sizes)):
                self.resblocks.append(
                    ResNetBlock(channels=resnet_blocks_channels, kernel_size=kernel, dilation=dilation)
                )
